...
def enter_user(user):
    ...
    usernameElement = driver.find_element_by_name("admin_username")
...
def get_dsl_status():
    ...
    while ctr < 4:
        try:
            dslUpstreamElement = driver.find_element_by_xpath("//span[@id='uspeed']")
    ...
    if ctr == 0:
        dslDownstreamElement = driver.find_element_by_xpath("//span[@id='dspeed']")
        sample['dslDownstreamElement'] = float(refp(dslDownstreamElement.text))

        dslLineStatusElement = driver.find_element_by_xpath("//span[@id='LINESTATUS']")
...
def get_dsl_transport():
    ...
    while ctr < 4:
        try:
            Packets_Downstream = driver.find_element_by_xpath("//span[@id='dsldpkt']")
    ...
    if ctr == 0:
        Packets_Upstream = driver.find_element_by_xpath("//span[@id='dslupkt']")
        sample['Packets_Upstream'] = int(Packets_Upstream.text)

        Total_Usage_Downstream = driver.find_element_by_xpath("//span[@id='dslRxByteTotal']")
        sample['Total_Usage_Downstream'] =  float(refp(Total_Usage_Downstream.text))

        Total_Usage_Upstream = driver.find_element_by_xpath("//span[@id='dslTxByteTotal']")
...
def get_dsl_channel():
    ...
    while ctr < 6:
        try:
            RS_FEC = driver.find_element_by_xpath("//span[@id='FEC_NEAR']")
    ...
    if ctr == 0:
        CRC_Errors = driver.find_element_by_xpath("//span[@id='CRC_NEAR']")
...
def get_dsl_power():
    ...
    while ctr < 6:
        try:
            SNR_downstream = driver.find_element_by_xpath("//span[@id='DSLSNRDOWN']")
    ...
    if ctr == 0:
        SNR_upstream = driver.find_element_by_xpath("//span[@id='DSLSNRUP']")
        sample['SNR_upstream'] =  float(refp(SNR_upstream.text))

        Power_downstream = driver.find_element_by_xpath("//span[@id='DSLPOWERDOWN']")
        sample['Power_downstream'] = float(refp(Power_downstream.text))

        Power_upstream = driver.find_element_by_xpath("//span[@id='DSLPOWERUP']")
...
def get_dsl_link():
    ...
    while ctr < 4:
        try:
            LinkUptime = driver.find_element_by_xpath("//span[@id='DSLUPTIME']")
    ...
    if ctr == 0:
        LinkTrainErrors = driver.find_element_by_xpath("//span[@id='linkTrainErr']")
...
def get_internet_settings():
    ...
    while ctr < 6:
        try:
            internetConnection = driver.find_element_by_xpath("//span[@id='ISPSTATS']")
    ...
    time.sleep(2)
    if ctr == 0 :
        sessionTime = driver.find_element_by_xpath("//span[@id='ppptimer']")
        sample['sessionTime'] =  sessionTime.text

        packetsSent = driver.find_element_by_xpath("//span[@id='pkttx']")
        sample['packetsSent'] = int(packetsSent.text)
        
        packetsReceived = driver.find_element_by_xpath("//span[@id='pktrx']")
...
def gotoInternet():
    ...
    while ctr < 4:
        try:
            goDsl1Status_button = driver.find_element_by_xpath("//a[contains(text(),'Internet Status')]").click()
            return 1
        except NoSuchElementException as Exception:
            ctr += 1
            time.sleep(1)
...
def gotoUtilities():
    ...
    while ctr < 4:
        try:
            goDsl1Status_button = driver.find_element_by_xpath("//a[contains(text(),'Utilities')]").click()
            return 1
        except NoSuchElementException as Exception:
            ctr += 1
            time.sleep(1)
...
def domodemStatusMain():
    ...
    while ctr < 6:
        try:
            goDsl1Status_button = driver.find_element_by_xpath("//a[contains(text(),'Modem Status')]").click()
...
def gotoDsl1():
    ...
    while ctr < 4:
        try:
            goDsl1Status_button = driver.find_element_by_xpath("//a[contains(text(),'DSL Status')]").click()
            return 1
        except NoSuchElementException as Exception:
            ctr += 1
            time.sleep(1)
...
def maybeClearDsl1(clear):
    ...
    if clear == 'yes':
        clear_button = driver.find_element_by_xpath("//a[@id='clear_btn']").click()
        #time.sleep(2)
...
def maybeReboot(reboot):
    ...
    if reboot == 'yes':
        ...
        time.sleep(2)
        popup_ok_button = driver.find_element_by_xpath("//a[@id='confirm_btn']").click()
        return 1
    else:
        return 0
...
def gotoResourcetable():
    ...
    while ctr < 4:
        try:
            goresourcetable_button = driver.find_element_by_xpath("//a[contains(text(),'Resource Table')]").click()
            return 1
        except NoSuchElementException as Exception:
            ctr += 1
            time.sleep(1)
...
def get_resourcetable(ts):
    ...
    #print("found endpoint  rows:", len(stuff))
    ...
    for row in stuff:
        ...
        if arow[1] == 'IGMP':
            arow.insert(2,'127.0.0.1')
            #print(arow, "gotta IGMP ... fixed it")
        elif len(arow) != 6:
            #print("problem endpoint arow:", arow)
            continue
        j = 0
        for varx in arow:
            stats[keys[j]] = varx  
